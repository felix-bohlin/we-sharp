<template>
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    viewBox="0 0 32 32"
  >
    <g fill="none">
      <path
        fill="#FFB02E"
        d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14c0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999Z"
      ></path>
      <path
        fill="#FFB02E"
        d="M10 21.5v-3.06A1.44 1.44 0 0 0 8.56 17c-.84 0-1.505.718-1.517 1.559C7.023 19.92 6.835 21.722 6 22c-3 1-4 5.5-1.5 7.5c2 1.6 4 1.5 5.5 1.5h2.764a1.236 1.236 0 0 0 .553-2.342L13 28.5h.72a1 1 0 0 0 .97-.758l.067-.272A.78.78 0 0 0 14 26.5l.33-.165a1.214 1.214 0 0 0 0-2.17L14 24h1.75a1.25 1.25 0 1 0 0-2.5H10Z"
      ></path>
      <path
        fill="#fff"
        d="M10.42 16.224a4.206 4.206 0 1 0 0-8.411a4.206 4.206 0 0 0 0 8.411Zm11.148.077a4.244 4.244 0 1 0 0-8.489a4.244 4.244 0 0 0 0 8.49Z"
      ></path>
      <path
        fill="#FF822D"
        d="M10 21.5v-3.06A1.44 1.44 0 0 0 8.56 17c-.84 0-1.505.718-1.517 1.559C7.023 19.92 6.835 21.722 6 22c-3 1-4 5.5-1.5 7.5c2 1.6 4 1.5 5.5 1.5h2.764a1.236 1.236 0 0 0 .553-2.342L13 28.5h.72a1 1 0 0 0 .97-.758l.067-.272A.78.78 0 0 0 14 26.5l.33-.165a1.214 1.214 0 0 0 0-2.17L14 24h1.75a1.25 1.25 0 1 0 0-2.5H10Z"
      ></path>
      <path
        fill="#402A32"
        d="M7.146 4.146C7.543 3.75 8.63 3 10 3s2.457.75 2.854 1.146a.5.5 0 0 1-.708.708C11.876 4.584 11.03 4 10 4c-1.03 0-1.877.584-2.146.854a.5.5 0 1 1-.708-.708Zm11.034 3.97C18.65 7.724 19.92 7 21.5 7c1.58 0 2.85.724 3.32 1.116a.5.5 0 0 1-.64.768C23.85 8.61 22.8 8 21.5 8c-1.3 0-2.35.61-2.68.884a.5.5 0 0 1-.64-.768ZM13 11.5a3 3 0 1 1-6 0a3 3 0 0 1 6 0Z"
      ></path>
      <path
        fill="#402A32"
        d="M24 11.5a3 3 0 1 1-6 0a3 3 0 0 1 6 0Zm-7.919 7.486a2.733 2.733 0 0 0-2.374.721a1 1 0 0 1-1.414-1.414a4.732 4.732 0 0 1 4.126-1.279c1.962.336 2.973 1.457 3.488 2.565a1 1 0 0 1-1.814.842c-.28-.604-.791-1.227-2.012-1.435Z"
      ></path>
    </g>
  </svg>
</template>
