    ...
    <div v-if="!uiStore.modalModeActive" md:hidden sticky left-0 right-0 bottom-0 z-10 bg-main pb="[env(safe-area-inset-bottom)]" transition="padding 20">
      ...
      >
        <div flex-1 grid grid-cols-5 gap-x-2 items-center text-xl>
          <NuxtLink to="/" grid active-class="text-primary" place-items-center text-xs gap-1>
            <span i-mdi-home text-3xl />
          </NuxtLink>
          <NuxtLink to="/stats" active-class="text-primary" grid place-items-center text-xs gap-1>
            <span i-mdi-chart-line text-3xl />
          </NuxtLink>

          <NuxtLink to="/add" active-class="text-primary" grid place-items-center text-xs gap-1>
            <span i-mdi-plus-circle text-3xl />
          </NuxtLink>

          <NuxtLink to="/group" active-class="text-primary" grid place-items-center text-xs gap-1>
            <span i-mdi-account-group text-3xl />
          </NuxtLink>

          <NuxtLink to="/profile" active-class="text-primary" grid place-items-center text-xs gap-1>
            <Avatar max-w-8 />
          </NuxtLink>
        </div>
      </nav>
